...
interface Props {
  tasks: TaskItem[]; sort: ReturnType<typeof useTaskSort>; totalTasks: number; confirmDelete: boolean; appearance: 'light' | 'dark' | 'space'; loading: boolean; error: string; onCreate: () => void
...
export function TaskWorkspaceV2(props: Props) {
  const { tasks, sort, totalTasks, confirmDelete, appearance, loading, error, onCreate, onSave, onDelete, onAttach, onDeleteAttachment, onViewAttachment, onSetReminder } = props
  ...
  return (
    <div className="relative p-5 sm:p-8 lg:min-h-0 lg:flex-1 lg:overscroll-contain lg:overflow-y-auto lg:p-10">
      ...
      <header className="relative mb-6">
        <p className="workspace-kicker text-xs font-bold uppercase tracking-[0.2em] text-indigo-600">{appearance === 'space' ? 'Mission dashboard' : 'Task workspace'}</p>
        <h1 className="workspace-title mt-2 text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white sm:text-4xl">{appearance === 'space' ? 'Navigate your next mission.' : 'Make progress today.'}</h1>
        <p className="workspace-subtitle mt-2 text-sm text-slate-500 dark:text-slate-400">{appearance === 'space' ? 'Plot priorities, track deadlines, and keep every objective in orbit.' : 'Organize priorities, meet deadlines, and keep momentum.'}</p>
      </header>
      ...
      <div className="relative"><TaskList tasks={tasks} viewMode={viewMode} confirmDelete={confirmDelete} loading={loading} onSave={onSave} onDelete={onDelete} onAttach={onAttach} onDeleteAttachment={onDeleteAttachment} onViewAttachment={onViewAttachment} onSetReminder={onSetReminder} /></div>
    </div>
  )
}
